 ...
 * 
 * @filesource testAutomationScripts.php
 * 
 */
...
$system = new tlCodeTracker($db);
$codeRepo = $system->getInterfaceObject($args->tproject_id);
$cfgObj = $codeRepo->getCfg();
$repo = $codeRepo->getAPIClient();
$params = ['id' => $cfgObj->projectidentifier, 
           'path' => $cfgObj->testscriptsbasepath];

$params['recursive'] = false;
$params['itemType'] = 'tree';
$gui->topContainers = $repo->getRepoFilesTreeFlat($params);

$gui->scripts = [];
foreach ($gui->topContainers as $path) {
  $params['itemType'] = 'blob';
  $params['recursive'] = true;
  $params['path'] = $path;
  $gui->scripts[$path] = $repo->getRepoFilesTreeFlat($params);
}

$gui->scriptQty = [];
$gui->totalScripts = 0;
foreach ($gui->scripts as $path => $elem) {
  foreach ($elem as $idx => $script_file) {
    if (strripos($script_file, '.js') === FALSE) {
      unset($gui->scripts[$path][$idx]);
    } else {
      $gui->scripts[$path][$idx] = 
        trim(str_ireplace($path, '', $gui->scripts[$path][$idx]), '/');
    }
  }
  $gui->scriptQty[$path] = count($gui->scripts[$path]);
  $gui->totalScripts += $gui->scriptQty[$path];
}
...
function initializeGui(&$dbHandler,$argsObj,&$tplanMgr) {
  ...
  $gui->title = lang_get('report_test_automation_scripts');
  $gui->do_report = array();
